...
namespace Game.Tests
{
    public class TestController : MonoBehaviour, IController
    {
        ...
        private void TestQueryStatus()
        {
            ...
            var playerModel = this.GetModel<PlayerModel>();
            var maxEnergy = playerModel.GetMaxEnergy();
            var nextLevelExp = playerModel.GetExpForNextLevel(playerModel.Level.Value);
            
            Debug.Log($"<color=cyan>玩家状态</color>");
            Debug.Log($"  等级: <color=white>{playerModel.Level.Value}</color>");
            Debug.Log($"  经验: <color=white>{playerModel.Exp.Value}</color> / {nextLevelExp} (升级需要)");
        ...
        private void OnInventoryUpdated(InventoryUpdatedEvent e)
        {
            ...
            if (e.Inventory.items != null)
            {
                ...
                {
                    var itemConfig = this.GetSystem<ConfigSystem>().Get<ItemConfig>(item.itemId);
        ...
        private void DrawPanelWindow(int windowId)
        {
            ...
            var nextExp = playerModelRef.GetExpForNextLevel(_cachedLevel);
            var expPercent = (float)_cachedExp / nextExp * 100;
            GUI.Label(new Rect(0, y, itemWidth, lineHeight), $"<color=#00CED1>经验:</color> {_cachedExp} / {nextExp} ({expPercent:F1}%)", _labelStyle);
            y += lineHeight;
